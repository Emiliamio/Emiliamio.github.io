...
## 多行异常堆栈合并解析——解决单行正则的致命痛点

传统的日志正则解析器最大的痛点在于遇到 Java 异常堆栈时，会将每一行 `\tat com.xxx` 当成一条损坏的孤立日志，导致日志总数暴增、结构错乱。

我为其设计了**智能多行聚合状态机**：
- 以行首是否匹配标准时间戳作为新记录的判定边界。
- 当遇到以 `\tat `、`at `、`Caused by:` 或空白缩进开头的延续行时，状态机自动将其聚合拼接到当前记录的 `detail` 字段中。
- 完整保留了异常堆栈调用链路，又确保了数据库层面的单条业务记录原子性。
...
我还做了一个纯前端的数据看板（LogScope）——拖放上传日志文件，浏览器端 JavaScript 直接解析和渲染，不依赖任何后端服务。设计上采用了暖白 + 米色基调，水平分布条代替常见的饼图/柱图——更像数据分析师的终稿报告，而不是监控中心的实时大屏。
